...
document.getElementById("days").addEventListener("click", function(e) {
    if (e.target && e.target.matches("li")&&(e.target.innerText != 0)) { 
		...
		d = (selected_month+1) < 10 ? "0"+(selected_month+1) : (selected_month+1);
		m = (e.target.innerText).length == 1 ? "0"+(e.target.innerText) : (e.target.innerText);
		window.location.href = "gallery.html?date=" + d + "-" + m + "-" + selected_year;
